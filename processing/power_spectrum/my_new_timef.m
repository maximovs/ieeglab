...
function [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = my_new_timef( data, frames, tlimits, Fs, varwin, varargin);

display('INNNN mynewtimef')
...
if ~iscell(data)
    display('data is not cell')
    display('***********************')
    frames
    display('data size BEFORE')
    size(data)
    data = reshape_data(data, frames);
    trials = size(data,ndims(data));
    display('data size AFTER')
    size(data)
    display('trials size')
    size(trials)
    display('***********************')
else
    display('data IS cell')
...
if strcmpi(g.lowmem, 'on') && numel(data) ~= g.frames && isempty(g.nfreqs) && ~iscell(data)
    ...
    % plot results 
    %-------------
    %myplottimef(P, R, Pboot, Rboot, ERP, freqsout, timesout, mbase, [], [], g);
...
if iscell(data)
    ...
    if strcmpi(g.plotersp, 'on') | strcmpi(g.plotitc, 'on')
        ...
        g = myplottimef(P1, R1, Pboot1, Rboot1, mean(data{1},2), freqs, timesout, mbase{1}, [], [], g);
        g.itcavglim = [];

        subplot(1,3,2); % plot Condition 2
        g.title = g.titleall{2};
        myplottimef(P2, R2, Pboot2, Rboot2, mean(data{2},2), freqs, timesout, mbase{2}, [], [], g);
    ...
    if isnan(g.alpha)
        ...
        if strcmpi(g.plotersp, 'on') | strcmpi(g.plotitc, 'on')
            myplottimef(P1-P2, Rdiff, [], [], mean(data{1},2)-mean(data{2},2), freqs, timesout, meanmbase, [], [], g);
        end;
    else
        ...
        % same as below: myplottimef(P1-P2, R2-R1, 10*resimages{1}, resimages{2}, mean(data{1},2)-mean(data{2},2), freqs, times, mbase, g);
        if strcmpi(g.plotersp, 'on') | strcmpi(g.plotitc, 'on')
            g.erspmax = []; % auto scale
            g.itcmax  = []; % auto scale
            myplottimef(10*resdiff{1}, resdiff{2}, 10*resimages{1}, resimages{2}, ...
...
if isempty(g.precomputed)
    ...
    display('ABOUT TO COMPUTE TIMEFREQ')
    ...
    display('alltfX calculated - size:')
    size(alltfX)
else
...
if ~isnan(g.alpha) | ~isempty(find(~isnan(g.pboot))) | ~isempty(find(~isnan(g.rboot)))% if bootstrap analysis requested . . .
    ...
    if ~isempty(find(~isnan(g.pboot))) % if ERSP bootstrap limits provided already
        ...
        if strcmpi(g.boottype, 'shuffle')
            ...
            display('P size before bootstat')
            size(P)
...
% -----------------
PA = P;
display('P size in average power')
size(P)
...
if ~isnan(g.alpha)
    if isempty(find(~isnan(g.pboot))) % if ERSP lims not provided
        if ndims(Pboottrials) < 3, Pboottrials = Pboottrials'; end;
            display('P size before compute pvals')
            size(P)
            display('Pboottrials size before compute pvals')
            size(Pboottrials)
...
if strcmpi(g.scale, 'abs') && strcmpi(g.basenorm, 'off') % % of baseline
        ...
        if g.erspmax > 1
            display('IN')
             g.erspmax = [1-(g.erspmax-1) g.erspmax];
             %Mi modificacion
             %g.erspmax = [1-(g.erspmax) g.erspmax-1];
        else g.erspmax = [g.erspmax 1+(1-g.erspmax)];
     	end;
    end
...
if strcmpi(g.plotersp, 'on') || strcmpi(g.plotitc, 'on')
    ...
    if strcmpi(g.plottype, 'image')
        %myplottimef('',P, R, Pboot, Rboot, ERP, freqs, timesout, mbase, maskersp, maskitc, g);        
    else
...
% plotting function
% -----------------
function g = plottimef0(P, R, Pboot, Rboot, ERP, freqs, times, mbase, maskersp, maskitc, g);
%
% compute ERP
%
