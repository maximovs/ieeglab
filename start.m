...
% Last Modified by GUIDE v2.5 26-May-2016 09:09:43
...
handles.data_status = 0;

handles.preprocessing_functions = cell(0);
handles.processing_functions = cell(0);
handles.current_function = get_current_popup_string(handles.function_select_menu);
handles.current_processing_function = get_current_popup_string(handles.processing_select_menu);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[handles.file_name, handles.path] = uiputfile;
...
handles.current_function = get_current_popup_string(hObject);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.preprocessing_functions = [handles.preprocessing_functions; {handles.current_function, strsplit(get(handles.preprocessing_variables,'String'),',')}];
handles.data_status = 0;
load_handles(handles);
guidata(hObject,handles)
...
handles.current_function_to_delete = get(hObject,'Value');
% Save the handles structure.
guidata(hObject,handles)
...
if isfield(handles,'current_function_to_delete')
    if ~isempty(handles.preprocessing_functions(:))
        handles.preprocessing_functions(handles.current_function_to_delete,:) = [];
        handles.data_status = 0;
    end
    load_handles(handles);
    guidata(hObject,handles)
...
handles.epoching_function = get_current_popup_string(hObject);
handles.data_status = min(1,handles.data_status);
% Save the handles structure.
guidata(hObject,handles)
...
handles.current_processing_function = get_current_popup_string(hObject);
% Save the handles structure.
guidata(hObject,handles)
...
handles.processing_functions = [handles.processing_functions; {handles.current_processing_function, strsplit(get(handles.processing_variables,'String'),',')}];
load_handles(handles);
handles.data_status = min(2,handles.data_status);
guidata(hObject,handles)
...
handles.current_processing_function_to_delete = get(hObject,'Value');
% Save the handles structure.
guidata(hObject,handles)
...
if isfield(handles, 'current_processing_function_to_delete')
    handles.processing_functions(handles.current_processing_function_to_delete, :) = [];
    load_handles(handles);
    handles.data_status = min(2,handles.data_status);
    guidata(hObject,handles)
end
...
function [handles] = run_preprocessing(hObject, handles)
    t = 0:1/5:20;
    handles.data = sin(2*pi*t*300);
    result = execute_preprocessing(handles);
    handles.preprocessed_data = result;
    handles.data_status = 1;
    guidata(hObject,handles)
...
function [handles] = run_epoching(hObject, handles)
    ...
    handles.epoched_data = execute_epoching(handles);
    handles.data_status = 2;
    guidata(hObject,handles)
...
function [handles] = run_processing(hObject, handles)
    ...
    handles.processed_data = execute_processing(handles);
    handles.data_status = 3;
    guidata(hObject,handles)